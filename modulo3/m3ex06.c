...
#define CMD_SECOND_LINE       0xC0 // Endereço da linha 2 (0x80 | 0x40)
...
// Exercício 6: Escreve string e gerencia \n
void lcdWrite(char *str)
{
    // Enquanto não chegar ao fim da string (byte 0x00)
    while (*str)
    {
        // Verifica se é o caractere de nova linha
        if (*str == '\n')
        {
            // Move cursor para o início da segunda linha (Endereço 0x40)
            // Comando: 0x80 (Set Address) | 0x40 (Endereço) = 0xC0
            lcdWriteByte(CMD_SECOND_LINE, 0); // isChar = 0 (Comando)
        }
        else
        {
            // Escreve o caractere normal no display
            lcdWriteByte(*str, 1); // isChar = 1 (Dado)
        }
